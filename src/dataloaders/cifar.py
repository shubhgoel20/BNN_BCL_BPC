...
def get(data_path, seed=0, pc_valid=0.10):
    ...
    if not os.path.isdir(path):
        ...
        for t in data.keys():
            for s in ["train", "test"]:
                data[t][s]["x"] = torch.stack(data[t][s]["x"]).view(
                    -1, size[0], size[1], size[2]
                )
                data[t][s]["y"] = torch.LongTensor(
                    np.array(data[t][s]["y"], dtype=int)
                ).view(-1)
                torch.save(
                    data[t][s]["x"],
                    os.path.join(
                        os.path.expanduser(path), "data" + str(t) + s + "x.bin"
                    ),
                )
                torch.save(
                    data[t][s]["y"],
                    os.path.join(
                        os.path.expanduser(path), "data" + str(t) + s + "y.bin"
                    ),
                )
    ...
    for i in range(10):
        ...
        for s in ["train", "test"]:
            ...
            data[i][s]["x"] = torch.load(
                os.path.join(
                    os.path.expanduser(path), "data" + str(ids[i]) + s + "x.bin"
                )
            )
            data[i][s]["y"] = torch.load(
                os.path.join(
                    os.path.expanduser(path), "data" + str(ids[i]) + s + "y.bin"
                )
            )
