    ...
    def __init__(
        ...
        if transposed:
            self.weight_mu = nn.Parameter(
                torch.Tensor(in_channels, out_channels // groups, *kernel_size).normal_(
                    0.0, 0.1
                )
            )
        ...
        self.weight = VariationalPosterior(
            self.weight_mu, self.weight_rho, self.device
        ).to(self.device)
        ...
        if self.use_bias:
            ...
            self.bias_mu = nn.Parameter(
                torch.empty(
                    (self.out_channels), device=self.device, dtype=torch.float32
                ).normal_(0.0, 0.1),
                requires_grad=True,
            )
            ...
            self.bias = VariationalPosterior(
                self.bias_mu, self.bias_rho, self.device
            ).to(self.device)
        else:
...
class BayesianConv2D(_ConvNd):
    ...
    def forward(self, input, sample=False, calculate_log_probs=False):
        if self.mask_flag:
            self.weight = VariationalPosterior(
                self.pruned_weight_mu, self.weight_rho, self.device
            )
            # if self.use_bias:
