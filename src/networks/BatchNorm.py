    ...
    def __init__(
        ...
        if self.affine:
            ...
            self.weight_mu = nn.Parameter(
                torch.empty(
                    (self.num_features), device=self.device, dtype=torch.float32
                ).normal_(0.0, 0.1),
                requires_grad=True,
            )
        ...
        if self.track_running_stats:
            self.register_buffer(
                "running_mean", torch.zeros(self.num_features).normal_(0.0, 1.0)
            )
            self.register_buffer(
                "running_var", torch.zeros(self.num_features).normal_(0.0, 1.0)
            )
        else:
